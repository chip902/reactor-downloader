...
yargs
	...
	.options({
		...
		"api-key": {
			type: "string",
			describe:
				"For authentication using an Adobe I/O integration. Your API key (client ID). You can find this on the overview screen for the integration you have created within the Adobe I/O console (https://console.adobe.io).",
		},
		"client-secret": {
			type: "string",
			describe:
				...
				await inquirer.prompt([
					{
						...
						choices: properties.map((property) => ({
							name: property.relationships.company.attributes.name + "/" + property.attributes.name,
							value: property.id,
						})),
						validate: Boolean,
					},
